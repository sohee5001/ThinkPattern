			...
			.ready(
					function() {
						...
						$(".list-group-item")
								.on(
										"click",
										function(event) { //디자인 패턴 리스트를 클릭하면 함수 호출
											...
											$
													.ajax({
														...
														success : function(data) {
															...
															$("#patterns")
																	.empty(); //#patterns의 태그는 남기고 내부 요소들을 모두 비움, 이때 모든 하위태그들이 다 지워져서 그냥 html로 넣으면 들어가지 않음
															$("#patterns")
																	.append(
																			list_temp); // 문서 로드 후 저장했던 list_temp(태그들이 다 들어있음)를 추가하여 아래에 html로 치환할 태그들을 생성해줌
															$("#p_name")
																	.html(
																			"<h1 name='p_name'>"
																					+ data.p_name
															...
															$("#p_contents")
																	.append(
																			"<img src=../../resources/res/images/patterns/"
																					+ decodeURIComponent(data.p_name)
																					+ ".png"
																					+ " alt=패턴>");
															$("#p_strong")
																	.html(
															...
															$("#move")
																	.html(
																			"<hr /><a class='button small' data-toggle='modal' data-target='#move_node'><i class='fa fa-caret-square-o-right' aria-hidden='true'></i> Create Project </a>");
